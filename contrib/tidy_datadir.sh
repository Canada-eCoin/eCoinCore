...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Canada eCoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Canada eCoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Canada eCoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Canada eCoin 0.7 datadir."
    ;;
  3)
    echo "Detected Canada eCoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Canada eCoin 0.8 datadir."
    ;;
esac

Canada eCoin Core version 0.16.x is now available from:
...
or `canadaecoind`/`canadaecoin-qt` (on Linux).
...
Canada eCoin Core is extensively tested on multiple operating systems using
the Linux kernel, macOS 10.8+, and Windows Vista and later. Windows XP is not supported.

Canada eCoin Core should also work on most other Unix-like systems but is not
